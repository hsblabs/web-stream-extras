...
console.log(
    "Round-trip match:",
    decoded.length === message.length && decoded.every((b, i) => b === message[i]),
);
...
while (true) {
    ...
    const ok =
        value.length === original.length && value.every((b, i) => b === original[i]);
...
const [decodedFrames] = await Promise.all([readAllDecodedPromise, writeAllPromise]);
